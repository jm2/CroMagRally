...
static void NavigateCycler(const MenuItem* entry)
{
	...
	if (delta != 0)
	{
		...
		{
			PlayEffect_Parms(kSfxCycle, FULL_CHANNEL_VOLUME, FULL_CHANNEL_VOLUME, NORMAL_CHANNEL_RATE * 2/3 + (VisualRandomFloat2() * 0x3000));	// never the synced RNG: a network game's pause menu runs here
		}
...
enum
{
	...
	else
	{
		...
		*entry->floatRange.valuePtr = newValue;
		...
		pitch += VisualRandomFloat() * 0.2f;							// never the synced RNG (see NavigateCycler)
