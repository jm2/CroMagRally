...
ObjNode *MakeFadeEvent(Boolean fadeIn) {
  ...
  NewObjectDefinitionType newObjDef = {
      ...
      .scale = 1,
      .flags =
          STATUS_BITS_FOR_2D | STATUS_BIT_OVERLAYPANE | STATUS_BIT_MOVEINPAUSE,
  ...
  float fadeDuration =
      fadeIn ? gGameView->fadeInDuration : gGameView->fadeOutDuration;
...
void OGL_FadeOutScene(void (*drawCall)(void), void (*moveCall)(void)) {
  ...
  while (!faderData->done) {
    ...
    if (faderData->frameCounter ==
        pFaderFrameCount) // fader wasn't moved by moveCall
    {
      MoveFadeEvent(fader);
...
void Enter2D(Boolean pauseDSp) { (void)pauseDSp; }
  ...
  if (!gGamePrefs.fullscreen) {
    ...
    if (enforceDisplayPref) {
      ...
      if (currentDisplay != desiredDisplay) {
        // We must switch back to windowed mode for the preferred monitor to
        // take effect
