...
ObjNode *MakeNewObject(NewObjectDefinitionType *newObjDef) {
  ...
  _Static_assert(0 == (offsetof(struct ObjNode, SpecialPadding) & 7),
                 "SpecialPadding should be aligned to 8");
  ...
  if (flags & STATUS_BIT_ONSPLINE) {
    ...
    newNodePtr->SplineMoveCall =
        newObjDef->moveCall; // save spline move routine
  } else {
  ...
  newNodePtr->Coord = newNodePtr->InitCoord = newNodePtr->OldCoord =
      newObjDef->coord; // save coords
  ...
  /*	newNodePtr->Flag[0] =
          ...
          newNodePtr->CType =
     // must init ctype to something ( INVALID_NODE_FLAG might be set from last
     delete) newNodePtr->CBits = newNodePtr->Delta.x = newNodePtr->Delta.y =
          newNodePtr->Delta.z =
          newNodePtr->Rot.x =
          newNodePtr->Rot.z = 0;
  */
  ...
  newNodePtr->Scale.x = newNodePtr->Scale.y = newNodePtr->Scale.z =
      newObjDef->scale;
  ...
  /*
     ...
     // nothing attached as QD3D object yet newNodePtr->ShadowNode = nil;
          newNodePtr->BaseTransformObject = nil;
          newNodePtr->NumCollisionBoxes = 0;
          newNodePtr->CollisionBoxes = nil;
     // no collision boxes yet
  */
  ...
  newNodePtr->ColorFilter.r = newNodePtr->ColorFilter.g =
      newNodePtr->ColorFilter.b = newNodePtr->ColorFilter.a = 1.0;
  ...
  newNodePtr->StatusBits |=
      STATUS_BIT_DETACHED; // its not attached to linked list yet
...
void ResetDisplayGroupObject(ObjNode *theNode) {
  ...
  if (theNode->Type >=
      gNumObjectsInBG3DGroupList[theNode->Group]) // see if illegal
    DoFatalAlert("ResetDisplayGroupObject: type > gNumObjectsInGroupList[]!");

  AttachGeometryToDisplayGroupObject(
      theNode, gBG3DGroupList[theNode->Group]
                             [theNode->Type]); // attach geometry to group
}

/************************* ADD GEOMETRY TO DISPLAY GROUP OBJECT
 * ***********************/
//
// Attaches a geometry object to the BaseGroup object. MakeNewDisplayGroupObject
// must have already been called which made the group & transforms.
//

void AttachGeometryToDisplayGroupObject(ObjNode *theNode,
                                        MetaObjectPtr geometry) {
  MO_AppendToGroup(theNode->BaseGroup, geometry);
}

/***************** CREATE BASE GROUP **********************/
//
// The base group contains the base transform matrix plus any other objects you
// want to add into it. This routine creates the new group and then adds a
// transform matrix.
//
...
void CreateBaseGroup(ObjNode *theNode) {
  ...
  if ((theNode->Scale.x == 0) || (theNode->Scale.y == 0) ||
      (theNode->Scale.z == 0))
    DoFatalAlert("CreateBaseGroup: A scale component == 0");

  OGLMatrix4x4_SetScale(&scaleMatrix, theNode->Scale.x,
                        theNode->Scale.y, // make scale matrix
                        theNode->Scale.z);

  OGLMatrix4x4_SetRotate_XYZ(&rotMatrix, theNode->Rot.x,
                             theNode->Rot.y, // make rotation matrix
                             theNode->Rot.z);

  OGLMatrix4x4_SetTranslate(&transMatrix, theNode->Coord.x,
                            theNode->Coord.y, // make translate matrix
                            theNode->Coord.z);

  OGLMatrix4x4_Multiply(&scaleMatrix, // mult scale & rot matrices
                        &rotMatrix, &theNode->BaseTransformMatrix);

  OGLMatrix4x4_Multiply(&theNode->BaseTransformMatrix, // mult by trans matrix
                        &transMatrix, &theNode->BaseTransformMatrix);

  /* CREATE A MATRIX XFORM */

  transObject = MO_CreateNewObjectOfType(
      MO_TYPE_MATRIX, 0,
      &theNode->BaseTransformMatrix); // make matrix xform object
  if (transObject == nil)
  ...
  theNode->BaseTransformObject =
      transObject; // keep extra LEGAL ref (remember to dispose later)
}
...
void MoveObjects(void) {
  ...
  do {
    ...
    gNextNode = thisNodePtr->NextNode; // get next node now (cuz current node
                                       // might get deleted)

    if (gSimulationPaused &&
        !(thisNodePtr->StatusBits & STATUS_BIT_MOVEINPAUSE))
      goto next;
...
void DrawObjects(void) {
  ...
  playerNum =
      GetPlayerNum(gCurrentSplitScreenPane); // get the player # who's draw
                                             // context is being drawn
  ...
  cameraX =
      gGameView->cameraPlacement[gCurrentSplitScreenPane].cameraLocation.x;
  cameraZ =
      gGameView->cameraPlacement[gCurrentSplitScreenPane].cameraLocation.z;
  ...
  do {
    ...
    if (statusBits &
        (STATUS_BIT_ISCULLED | STATUS_BIT_HIDDEN)) // see if is culled or hidden
      goto next;

    if (statusBits &
        STATUS_BIT_ONLYSHOWTHISPLAYER) // see if only show for current player's
                                       // draw context
    {
      if (theNode->PlayerNum != playerNum)
        goto next;
    } else if (statusBits &
               STATUS_BIT_NOSHOWTHISPLAYER) // see if dont show for current
                                            // player's draw context
    {
      if (theNode->PlayerNum == playerNum)
        goto next;
    }
    ...
    if (gAutoFadeStartDist != 0.0f) // see if this level has autofade
    {
      if (statusBits & STATUS_BIT_AUTOFADE) {
        ...
        dist = CalcQuickDistance(cameraX, cameraZ, theNode->Coord.x,
                                 theNode->Coord.z); // see if in fade zone
    ...
    if ((statusBits & STATUS_BIT_CLIPALPHA) && (gGlobalTransparency == 1.0f)) {
      if (!clipAlpha) {
        glAlphaFunc(
            GL_EQUAL,
            1); // draw any pixel who's Alpha == 1, skip semi-transparent pixels
        clipAlpha = true;
      }
    } else if (clipAlpha) {
      clipAlpha = false;
    ...
    if (statusBits & STATUS_BIT_AIMATCAMERA) {
      theNode->Rot.y =
          PI + CalcYAngleFromPointToPoint(theNode->Rot.y, theNode->Coord.x,
                                          theNode->Coord.z, cameraX, cameraZ);
    ...
    if (noLighting ||
        (theNode->Scale.y == 1.0f)) // if scale == 1 or no lighting, then dont
    ...
    case SKELETON_GENRE:
      ...
      overrideTexture =
          theNode->Skeleton
              ->overrideTexture; // get any override texture ref (illegal ref)
      ...
      for (i = 0; i < numTriMeshes; i++) // submit each trimesh of it
      {
        if (overrideTexture) // set override texture
        {
          if (gLocalTriMeshesOfSkelType[skelType][i].numMaterials > 0) {
            MO_DisposeObjectReference(
                gLocalTriMeshesOfSkelType[skelType][i]
                    .materials[0]); // de-reference the existing texture
            gLocalTriMeshesOfSkelType[skelType][i].materials[0] =
                MO_GetNewReference(
                    overrideTexture); // put new ref to override in there
          }
        }
    ...
    case SPRITE_GENRE: {
      ...
      int spriteFlags = kTextMeshAlignCenter | kTextMeshAlignMiddle |
                        kTextMeshKeepCurrentProjection;
...
void DeleteObject(ObjNode *theNode) {
  ...
  if (theNode->CType == INVALID_NODE_FLAG) // see if already deleted
  {
    DoFatalAlert("Attempted to Double Delete an Object.  Object was already "
                 "deleted! genre=%d group=%d type=%d",
  ...
  switch (theNode->Genre) {
  case SKELETON_GENRE:
    FreeSkeletonBaseData(
        theNode->Skeleton); // purge all alloced memory for skeleton data
  ...
  if (theNode->TerrainItemPtr) {
    theNode->TerrainItemPtr->flags &=
        ~ITEM_FLAGS_INUSE; // clear the "in use" flag
  }
  ...
  if (gNumObjsInDeleteQueue >=
      OBJ_DEL_Q_SIZE) // object delete queue full -- OK inbetween scenes but
...
void DetachObject(ObjNode *theNode) {
  ...
  if (theNode->StatusBits &
      STATUS_BIT_DETACHED) // make sure not already detached
    return;
...
void AttachObject(ObjNode *theNode) {
  ...
  else if (slot < gFirstNodePtr->Slot) {
  ...
  /* SCAN FOR INSERTION PLACE */
    ...
    scanNodePtr =
        gFirstNodePtr
            ->NextNode; // start scanning for insertion slot on 2nd node
...
void DisposeObjectBaseGroup(ObjNode *theNode) {
  ...
  if (theNode->BaseTransformObject !=
      nil) // also nuke extra ref to transform object
  {
...
void UpdateObjectTransforms(ObjNode *theNode) {
  ...
  OGLMatrix4x4_SetScale(&m, theNode->Scale.x, theNode->Scale.y,
                        theNode->Scale.z);
  ...
  /* STANDARD XYZ ROTATION */
    ...
    OGLMatrix4x4_SetRotate_XYZ(&m2, theNode->Rot.x, theNode->Rot.y,
                               theNode->Rot.z);
  }
...
ObjNode *GetNthChainedNode(ObjNode *start, int targetIndex,
                           ObjNode **outPrevNode) {
  ObjNode *pNode = NULL;
  ObjNode *node = start;
  ...
  if (start != NULL) {
    int currentIndex;
    for (currentIndex = 0; (node != NULL) && (currentIndex < targetIndex);
         currentIndex++) {
      pNode = node;
      node = pNode->ChainNode;

      if (pNode && node) {
        GAME_ASSERT_MESSAGE(pNode->Slot <= node->Slot,
                            "the game assumes that chained nodes are sorted "
                            "after their parent");
      }
    }
