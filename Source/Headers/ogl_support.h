...
/* 3x3 MATRIX INDECIES */
enum { N00 = 0, N10, N20, N01, N11, N21, N02, N12, N22 };
...
typedef struct {
  ...
  OGLCameraPlacement
      cameraPlacement[MAX_VIEWPORTS]; // 2 cameras, one for each viewport/player
  float fov[MAX_VIEWPORTS], hither, yon;
  Boolean useFog;
  Boolean clearBackBuffer;
  float pillarboxRatio;
  Boolean fadePillarbox; // if true, pillarbox border brightness tracks global
                         // gamma fade
  Boolean
      fadeSound; // if true, global sound volume will track global gamma fade
