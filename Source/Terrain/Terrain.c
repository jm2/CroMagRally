...
#define ITEM_WINDOW                                                            \
  SUPERTILE_ITEMRING_MARGIN // # supertiles for item add window (must be
                            // integer)
#define OUTER_SIZE                                                             \
  0 // 0.6f						// size of border out of
    // add window for delete window (can be float)
...
long gTerrainUnitWidth, gTerrainUnitDepth; // width & depth of terrain in world
                                           // units (see TERRAIN_POLYGON_SIZE)
...
long gNumSuperTilesDeep,
    gNumSuperTilesWide; // dimensions of terrain in terms of supertiles
static long gCurrentSuperTileRow[MAX_PLAYERS],
    gCurrentSuperTileCol[MAX_PLAYERS];
static long gPreviousSuperTileCol[MAX_PLAYERS],
    gPreviousSuperTileRow[MAX_PLAYERS];
...
void InitCurrentScrollSettings(void) {
  ...
  for (i = 0; i < gNumTotalPlayers;
       i++) // init settings for each player in game
  {
    x = gPlayerInfo[i].coord.x -
        (SUPERTILE_ACTIVE_RANGE * SUPERTILE_SIZE * TERRAIN_POLYGON_SIZE);
    y = gPlayerInfo[i].coord.z -
        (SUPERTILE_ACTIVE_RANGE * SUPERTILE_SIZE * TERRAIN_POLYGON_SIZE);
    GetSuperTileInfo(x, y, &gCurrentSuperTileCol[i], &gCurrentSuperTileRow[i],
                     &dummy1, &dummy2);
...
void InitSuperTileGrid(void) {
  ...
  Alloc_2d_array(SuperTileStatus, gSuperTileStatusGrid, gNumSuperTilesDeep,
                 gNumSuperTilesWide); // alloc 2D grid array
...
//
// Preallocates memory and initializes the data for the maximum number of
// supertiles that we will ever need on this level.
//
...
void CreateSuperTileMemoryList(void) {
  ...
  if (gSuperTileMeshData == nil)
    DoFatalAlert(
        "CreateSuperTileMemoryList: AllocPtr failed - gSuperTileMeshData");

  /* ALLOC POINTS FOR ALL SUPERTILES */

  gSuperTileCoords = AllocPtr(sizeof(OGLPoint3D) *
                              (NUM_VERTICES_IN_SUPERTILE * MAX_SUPERTILES));
  if (gSuperTileCoords == nil)
    DoFatalAlert(
        "CreateSuperTileMemoryList: AllocPtr failed - gSuperTileCoords");

  /* ALLOC UVS FOR ALL SUPERTILES */

  gSuperTileUVs = AllocPtr(sizeof(OGLTextureCoord) * NUM_VERTICES_IN_SUPERTILE *
                           MAX_SUPERTILES);
  ...
  gSuperTileTriangles = AllocPtr(sizeof(MOTriangleIndecies) *
                                 NUM_TRIS_IN_SUPERTILE * MAX_SUPERTILES);
  if (gSuperTileTriangles == nil)
    DoFatalAlert(
        "CreateSuperTileMemoryList: AllocPtr failed - gSuperTileTriangles");
  ...
  for (i = 0; i < MAX_SUPERTILES; i++) {
    ...
    meshPtr->numMaterials =
        -1; // textures will be manually submitted in drawing function!
    ...
    for (v = 0; v <= SUPERTILE_SIZE; v++) {
      for (u = 0; u <= SUPERTILE_SIZE; u++) {
        uvPtr[j].u =
            (float)u /
            (float)
                SUPERTILE_SIZE; // sets uv's 0.0 -> 1.0 for single texture map
...
static uint16_t BuildTerrainSuperTile(long startCol, long startRow) {
  ...
  superTilePtr->x =
      (startCol * TERRAIN_POLYGON_SIZE) +
      (TERRAIN_SUPERTILE_UNIT_SIZE / 2); // also remember world coords
  superTilePtr->z =
      (startRow * TERRAIN_POLYGON_SIZE) + (TERRAIN_SUPERTILE_UNIT_SIZE / 2);

  superTilePtr->left =
      (startCol * TERRAIN_POLYGON_SIZE); // also save left/back coord
  ...
  meshData =
      gSuperTileMemoryList[superTileNum].meshData; // get ptr to mesh data
  triangleList = meshData->triangles; // get ptr to triangle index list
  vertexPointList = meshData->points; // get ptr to points list

  GAME_ASSERT_MESSAGE(meshData->colorsByte == NULL,
                      "per-vertex colors unsupported in CMR terrain! (byte)");
  GAME_ASSERT_MESSAGE(meshData->colorsFloat == NULL,
                      "per-vertex colors unsupported in CMR terrain! (float)");
  ...
  for (row2 = 0; row2 <= SUPERTILE_SIZE; row2++) {
    ...
    for (col2 = 0; col2 <= SUPERTILE_SIZE; col2++) {
      ...
      if ((row >= gTerrainTileDepth) ||
          (col >= gTerrainTileWidth)) // check for edge vertices (off map array)
        height = 0;
      else
      ...
      gWorkGrid[row2][col2].point.y =
          height; // save height @ this tile's upper left corner

      /* SET UV */

      gWorkGrid[row2][col2].uv.u =
          (float)col2 *
          (1.0f /
           (float)
               SUPERTILE_SIZE); // sets uv's 0.0 -> 1.0 for single texture map
      gWorkGrid[row2][col2].uv.v =
          1.0f - ((float)row2 * (1.0f / (float)SUPERTILE_SIZE));

      /* SET COLOR */

      gWorkGrid[row2][col2].color.r = gWorkGrid[row2][col2].color.g =
          gWorkGrid[row2][col2].color.b = gWorkGrid[row2][col2].color.a = 1.0;
  ...
  for (row = 0; row < (SUPERTILE_SIZE + 1); row++) {
    for (col = 0; col < (SUPERTILE_SIZE + 1); col++) {
      vertexPointList[numPoints] =
          gWorkGrid[row][col].point; // copy from work grid
      numPoints++;
    }
  }
  ...
  for (row2 = 0; row2 < SUPERTILE_SIZE; row2++) {
    ...
    for (col2 = 0; col2 < SUPERTILE_SIZE; col2++) {
      ...
      if (gMapSplitMode[row][col] ==
          SPLIT_BACKWARD) // set coords & uv's based on splitting
      {
        /* \ */
  ...
  for (row = 0; row <= SUPERTILE_SIZE; row++) {
    for (col = 0; col <= (SUPERTILE_SIZE * 2); col += 2) {
      ...
      //
      // We use the face normal already calculated for triangles inside the
      // supertile, but for tiles/tris outside the supertile (on the borders),
      // we need to calculate the face normals there.
      //
      ...
      for (ro = -1; ro <= 0; ro++) {
        for (co = -2; co <= 0; co += 2) {
          ...
          if ((cc >= 0) && (cc < (SUPERTILE_SIZE * 2)) && (rr >= 0) &&
              (rr <
               SUPERTILE_SIZE)) // see if this vertex is in supertile bounds
          {
            n1 = &faceNormal[rr * (SUPERTILE_SIZE * 2) +
                             cc]; // average 2 triangles...
            n2 = n1 + 1;
          ...
          {
            CalcTileNormals(rr + startRow, (cc >> 1) + startCol, &nA,
                            &nB); // calculate the 2 face normals for this tile
      ...
      FastNormalizeVector(avX, avY, avZ,
                          &vertexNormalList[i]); // normalize the vertex normal
      i++;
    }
  }
  ...
  /*********************/
  //
  // This y coord is not used to translate since the terrain has no translation
  // matrix instead, this is used by the culling routine for culling tests
  //

  superTilePtr->y =
      (miny + maxy) * .5f; // calc center y coord as average of top & bottom

  superTilePtr->bBox.min.x = gWorkGrid[0][0].point.x;
  superTilePtr->bBox.max.x =
      gWorkGrid[0][0].point.x + TERRAIN_SUPERTILE_UNIT_SIZE;
  superTilePtr->bBox.min.y = miny;
  superTilePtr->bBox.max.y = maxy;
  superTilePtr->bBox.min.z = gWorkGrid[0][0].point.z;
  superTilePtr->bBox.max.z =
      gWorkGrid[0][0].point.z + TERRAIN_SUPERTILE_UNIT_SIZE;
...
/********************* DRAW TERRAIN **************************/
//
// This is the main call to update the screen.  It draws all ObjNode's and the
// terrain itself
//
...
void DrawTerrain(void) {
  ...
  cameraCoord =
      gGameView->cameraPlacement[gCurrentSplitScreenPane].cameraLocation;
  ...
  for (r = 0; r < gNumSuperTilesDeep; r++) {
    for (c = 0; c < gNumSuperTilesWide; c++) {
      ...
      if (gSuperTileStatusGrid[r][c].statusFlags &
          SUPERTILE_IS_USED_THIS_FRAME) // see if used
      {
#if TERRAIN_DEBUG_GWORLD
        if (gDebugMode == 2) {
          SetGWorld(gTerrainDebugGWorld, nil);
        ...
        glTexParameteri(
            ...
            GL_CLAMP_TO_EDGE); // set clamp mode after each texture set since
                               // OGL just likes it that way
        ...
        // Textures Enabled, Lighting/Culling Disabled for safety visibility
        {
          ...
          OGLPoint3D camPos =
              gGameView->cameraPlacement[gCurrentSplitScreenPane]
                  .cameraLocation;
          ...
          GLfloat viewRot[16];
          for (int k = 0; k < 16; k++)
            viewRot[k] = gLocalToViewMatrix.value[k];
          viewRot[12] = 0.0f;
          viewRot[13] = 0.0f;
          viewRot[14] = 0.0f;
          ...
          // Force White (Textured) - Ignoring vertex colors for now as they
          // might be dark
          ...
          for (int t = 0; t < data->numTriangles; t++) {
            ...
            // Vertex 0
            if (data->uvs)
              glTexCoord2f(data->uvs[i0].u, data->uvs[i0].v);
            // if (data->normals) glNormal3f(data->normals[i0].x,
            // data->normals[i0].y, data->normals[i0].z);
            glVertex3f(data->points[i0].x - originX,
                       data->points[i0].y - originY,
                       data->points[i0].z - originZ);

            // Vertex 1
            if (data->uvs)
              glTexCoord2f(data->uvs[i1].u, data->uvs[i1].v);
            glVertex3f(data->points[i1].x - originX,
                       data->points[i1].y - originY,
                       data->points[i1].z - originZ);

            // Vertex 2
            if (data->uvs)
              glTexCoord2f(data->uvs[i2].u, data->uvs[i2].v);
            glVertex3f(data->points[i2].x - originX,
                       data->points[i2].y - originY,
                       data->points[i2].z - originZ);
          }
          glEnd();
  ...
  if (gCurrentSplitScreenPane !=
      gNumSplitScreenPanes -
          1) // if splitscreen, then don't do this until done with last player
    goto dont_prep_grid;
  ...
  for (r = 0; r < gNumSuperTilesDeep; r++) {
    for (c = 0; c < gNumSuperTilesWide; c++) {
      ...
      if (gSuperTileStatusGrid[r][c].statusFlags &
          SUPERTILE_IS_DEFINED) // is it defined?
      {
        if (!(gSuperTileStatusGrid[r][c].statusFlags &
              SUPERTILE_IS_USED_THIS_FRAME)) // was it used?  If not, then
                                             // release the supertile definition
        {
      ...
      gSuperTileStatusGrid[r][c].statusFlags &=
          ~SUPERTILE_IS_USED_THIS_FRAME; // clear the isUsed bit
    }
  }
dont_prep_grid:;
}
...
float GetTerrainY(float x, float z) {
  ...
  if (gMapSplitMode[row][col] == SPLIT_BACKWARD) // if \ split
  {
    if (xi < zi) // which triangle are we on?
      CalcPlaneEquationOfTriangle(
          &planeEq, &p[0], &p[2],
          &p[3]); // calc plane equation for left triangle
    else
      CalcPlaneEquationOfTriangle(
          &planeEq, &p[0], &p[1],
          &p[2]); // calc plane equation for right triangle
  } else          // otherwise, / split
  {
    ...
    if (xi > zi)
      ...
      CalcPlaneEquationOfTriangle(
          &planeEq, &p[1], &p[2],
          &p[3]); // calc plane equation for right triangle
  }
...
float GetMinTerrainY(float x, float z, short group, short type, float scale) {
  ...
  bBox = &gObjectGroupBBoxList[group]
                              [type]; // get ptr to this model's bounding box
...
void GetSuperTileInfo(long x, long z, long *superCol, long *superRow,
                      long *tileCol, long *tileRow) {
  long row, col;
  ...
  col = x *
        (1.0f / TERRAIN_SUPERTILE_UNIT_SIZE); // calc supertile relative row/col
                                              // that the coord lies on
  ...
  *tileRow =
      row *
      SUPERTILE_SIZE; // return which tile row/col the super tile starts on
...
void DoPlayerTerrainUpdate(void) {
  ...
  static const Byte
      gridMask[(SUPERTILE_ACTIVE_RANGE + SUPERTILE_ITEMRING_MARGIN) * 2]
  ...
  for (playerNum = 0; playerNum < gNumTotalPlayers; playerNum++) {
    ...
    gCurrentSuperTileCol[playerNum] =
        x * TERRAIN_SUPERTILE_UNIT_SIZE_Frac; // top/left row,col
    gCurrentSuperTileRow[playerNum] = y * TERRAIN_SUPERTILE_UNIT_SIZE_Frac;

    /* SEE IF ROW/COLUMN HAVE CHANGED */

    deltaRow = SDL_abs(gCurrentSuperTileRow[playerNum] -
                       gPreviousSuperTileRow[playerNum]);
    deltaCol = SDL_abs(gCurrentSuperTileCol[playerNum] -
                       gPreviousSuperTileCol[playerNum]);

    if (deltaRow || deltaCol) {
      moved = true;
      if ((deltaRow > 1) ||
          (deltaCol > 1)) // if moved > 1 tile then need to do full item scan
        fullItemScan = true;
      else
        fullItemScan = false;
    } else {
    ...
    maxRow = gCurrentSuperTileRow[playerNum] + (SUPERTILE_ACTIVE_RANGE * 2) +
             SUPERTILE_ITEMRING_MARGIN;
    maxCol = gCurrentSuperTileCol[playerNum] + (SUPERTILE_ACTIVE_RANGE * 2) +
             SUPERTILE_ITEMRING_MARGIN;

    for (row = gCurrentSuperTileRow[playerNum] - SUPERTILE_ITEMRING_MARGIN,
        maskRow = 0;
         row < maxRow; row++, maskRow++) {
      if (row < 0) // see if row is out of range
        continue;
      if (row >= gNumSuperTilesDeep)
        break;

      for (col = gCurrentSuperTileCol[playerNum] - SUPERTILE_ITEMRING_MARGIN,
          maskCol = 0;
           col < maxCol; col++, maskCol++) {
        ...
        if (mask == 0)
          ...
          if (gPlayerInfo[playerNum].onThisMachine &&
              (!gPlayerInfo[playerNum]
                    .isComputer)) // only add supertile if this player is
                                  // actually on this machine
          {
            /* IS THIS SUPERTILE NOT ALREADY DEFINED? */

            if (!(gSuperTileStatusGrid[row][col].statusFlags &
                  SUPERTILE_IS_DEFINED)) {
              if (gSuperTileTextureGrid[row][col].superTileID !=
                  0) // supertiles with texture ID #0 are blank, so dont build
                     // them
              {
                gSuperTileStatusGrid[row][col].supertileIndex =
                    BuildTerrainSuperTile(
                        col * SUPERTILE_SIZE,
                        row * SUPERTILE_SIZE); // build the supertile
                gSuperTileStatusGrid[row][col].statusFlags =
                    SUPERTILE_IS_DEFINED |
                    SUPERTILE_IS_USED_THIS_FRAME; // mark as defined & used
              }
            } else
              gSuperTileStatusGrid[row][col].statusFlags |=
                  SUPERTILE_IS_USED_THIS_FRAME; // mark this as used
          }
        }
        ...
        {
          if (fullItemScan ||
              (mask == 2)) // if full scan or mask is 2 then check for items
          {
...
static void CalcNewItemDeleteWindow(int playerNum) {
  ...
  temp = gCurrentSuperTileCol[playerNum] *
         TERRAIN_SUPERTILE_UNIT_SIZE; // convert to unit coords
  temp2 = temp - (ITEM_WINDOW + OUTER_SIZE) *
                     TERRAIN_SUPERTILE_UNIT_SIZE; // factor window left
  gPlayerInfo[playerNum].itemDeleteWindow.left = temp2;

  /* CALC RIGHT SIDE OF WINDOW */

  temp += SUPERTILE_DIST_WIDE *
          TERRAIN_SUPERTILE_UNIT_SIZE; // calc offset to right side
  temp += (ITEM_WINDOW + OUTER_SIZE) *
          TERRAIN_SUPERTILE_UNIT_SIZE; // factor window right
  gPlayerInfo[playerNum].itemDeleteWindow.right = temp;

  /* CALC FAR SIDE OF WINDOW */

  temp = gCurrentSuperTileRow[playerNum] *
         TERRAIN_SUPERTILE_UNIT_SIZE; // convert to unit coords
  temp2 = temp - (ITEM_WINDOW + OUTER_SIZE) *
                     TERRAIN_SUPERTILE_UNIT_SIZE; // factor window top/back
  gPlayerInfo[playerNum].itemDeleteWindow.top = temp2;

  /* CALC NEAR SIDE OF WINDOW */

  temp += SUPERTILE_DIST_DEEP *
          TERRAIN_SUPERTILE_UNIT_SIZE; // calc offset to bottom side
  temp += (ITEM_WINDOW + OUTER_SIZE) *
          TERRAIN_SUPERTILE_UNIT_SIZE; // factor window bottom/front
...
void CalculateSplitModeMatrix(void) {
  ...
  Alloc_2d_array(Byte, gMapSplitMode, gTerrainTileDepth,
                 gTerrainTileWidth); // alloc 2D array
