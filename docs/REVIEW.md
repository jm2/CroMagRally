...
**tvOS**
- **Fixed: tvOS persistent saves.** Prefs, scoreboard, and tournament progression now use the `NSUserDefaults` bridge in `Source/System/TVOSStorage.m` through `LoadUserDataFile`/`SaveUserDataFile`. When no persistent copy exists, `LoadUserDataFile` reports a missing file and its callers initialize defaults; it does not migrate legacy files from Caches. Caches is no longer the save destination.
