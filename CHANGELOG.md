...
- **Unreleased**
    - Add a "CPU cars" option to multiplayer races (split-screen and LAN): empty slots fill with CPU cars, and only the humans' finishes end the race and decide who wins. In LAN games the host decides, and every peer seats the same cars and drivers. CPU weapon use in LAN games is decided by the host and applied at the same frame on every machine.
