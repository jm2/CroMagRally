...
- **Unreleased**
    ...
    - Split-screen sound effects are placed from every player's camera. With three or four players, a sound's volume only counted the first two cameras, and every pane used player 1's left/right direction.
    - Race records save the difficulty the race was played at, such as a LAN host's, instead of this machine's setting.
    ...
    - Debian packages built against the system SDL declare the SDL runtime package and minimum version they need, and a package that bundles the system's SDL declares the libraries that SDL links against. Packaging a source-built shared SDL without bundling it is now a configure error instead of a package with no SDL.
